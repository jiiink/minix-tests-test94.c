...
static uint32_t test94_fill_exact(int fd, uint8_t *buf, size_t size, uint32_t seq) {
    ...
    hdrlen = BPF_WORDALIGN(sizeof(struct bpf_hdr)) + sizeof(struct ip) + sizeof(struct udphdr) + sizeof(seq);
    ...
    chunk_size = len - hdrlen;

    while (size > 0) {
        memset(buf, 'Y', chunk_size);
        if (chunk_size > sizeof(seq)) {
            buf[sizeof(seq)] = 'X';
        }
        buf[chunk_size - 1] = 'Z';
        memcpy(buf, &seq, sizeof(seq));
        ssize_t written = write(fd, buf, chunk_size);
...
static void test94_fill_random(int fd, uint8_t *buf, size_t size) {
    ...
    for (left = (ssize_t)size; left >= 0; seq++) {
        ...
        ssize_t write_ret = write(fd, buf, payload_len);
        if (write_ret != (ssize_t)payload_len) {
            e(0);
            break;
        }
...
static void test94_add_random(int fd, uint8_t *buf, size_t size, uint32_t seq) {
    ...
    if (limit > 0) {
        long rand_val = lrand48();
        if (rand_val < 0) {
            rand_val = 0;
        }
        count = (size_t)(rand_val % limit);
    }

    test94_add_specific(fd, buf, count, seq);
}
...
{
    if (!buf || caplen < (ssize_t)(sizeof(struct ip) + sizeof(struct udphdr)))
        return -1;

    struct ip ip_hdr;
    memcpy(&ip_hdr, buf, sizeof(struct ip));

    if (ip_hdr.ip_v != IPVERSION)
        return -1;
    if (ip_hdr.ip_hl != (sizeof(struct ip) >> 2))
        return -1;
    if (ip_hdr.ip_p != IPPROTO_UDP)
        return -1;

    struct udphdr udp_hdr;
    memcpy(&udp_hdr, buf + sizeof(struct ip), sizeof(struct udphdr));

    if (udp_hdr.uh_sport != htons(TEST_PORT_A))
        return -1;
    if (udp_hdr.uh_dport != htons(TEST_PORT_B))
        return -1;
    if ((datalen < (ssize_t)sizeof(struct ip)) ||
        (datalen - (ssize_t)sizeof(struct ip) != ntohs(udp_hdr.uh_ulen)))
        return -1;
                            ...
                            uint32_t *caplen, uint32_t *datalen)
{
    ...
    while (len > 0) {
        ...
        if (bh.bh_hdrlen + BPF_WORDALIGN(bh.bh_caplen) > (uint32_t)len) e(0);
        ...
        if (bh.bh_caplen < (uint32_t)(off + sizeof(seq))) e(0);
        ...
        if ((uint32_t)off < bh.bh_caplen) {
            if ((uint32_t)off < bh.bh_caplen && (uint32_t)off < bh.bh_datalen - 1) {
                if (buf[off] != 'X') e(0);
                off++;
                for (; (uint32_t)off < bh.bh_caplen && (uint32_t)off < bh.bh_datalen - 1; off++) {
                    if (buf[off] != 'Y') e(0);
                }
            }
            if ((uint32_t)off < bh.bh_caplen && (uint32_t)off == bh.bh_datalen - 1) {
 ...
 */
static size_t test94_setup(int *fd, int *fd2, int *fd3, uint8_t **buf, unsigned int req_size, int set_filter)
{
    struct sockaddr_in sinA, sinB;
    struct ifreq ifr;
    struct bpf_program bf;
    unsigned int dlt, size = req_size;

    if (!fd || !fd2 || !fd3 || !buf) e(0);
    ...
    *buf = malloc(size);
    ...
    if (!LOOPBACK_IFNAME || strlcpy(ifr.ifr_name, LOOPBACK_IFNAME, sizeof(ifr.ifr_name)) >= sizeof(ifr.ifr_name)) e(0);
...
static void test94_cleanup(int fd, int fd2, int fd3, uint8_t *buf) {
    ...
    if (fd3 >= 0 && close(fd3) != 0) err = 1;
    if (fd2 >= 0 && close(fd2) != 0) err = 1;
    free(buf);
    if (fd >= 0 && close(fd) != 0) err = 1;
...
static void test94a(void)
{
    ...
    ssize_t len;
    uint32_t seq;
    int fd = -1, fd2 = -1, fd3 = -1, status = 0, bytes = 0, fl = 0;
    int wait_pid;
...
static void test94b(void)
{
    ...
    uint32_t seq;
    pid_t pid;
    int fd = -1, fd2 = -1, fd3 = -1, bytes = 0, status = 0, fl;
    int ret;
...
static void test94c(void)
{
    ...
    pid_t pid;
    uint32_t count, seq;
    int fd = -1, fd2 = -1, fd3 = -1, bytes = 0, status = 0, fl = 0;
...
static void test94d(void)
{
    ...
    size_t size;
    ssize_t len;
    int fd = -1, fd2 = -1, fd3 = -1, datalen;

    subtest = 4;

    size = test94_setup(&fd, &fd2, &fd3, &buf, 32768, 1);
    if (size != 32768) goto error;

    datalen = 65000;
    if (setsockopt(fd2, SOL_SOCKET, SO_SNDBUF, &datalen, sizeof(datalen)) != 0) goto error;
    ...
    buf2[0] = 'X';
    if ((size_t)(size - sizeof(struct udphdr) - sizeof(struct ip) - BPF_WORDALIGN(sizeof(bh)) - 1) >= datalen)
        goto error;
    buf2[size - sizeof(struct udphdr) - sizeof(struct ip) - BPF_WORDALIGN(sizeof(bh)) - 1] = 'Z';

    if (write(fd2, buf2, datalen) != datalen) goto error;
    if (read(fd, buf, size) != (ssize_t)size) goto error;

    memcpy(&bh, buf, sizeof(bh));
    if (bh.bh_hdrlen != BPF_WORDALIGN(sizeof(bh))) goto error;
    if (bh.bh_caplen != (size - BPF_WORDALIGN(sizeof(bh)))) goto error;
    if (bh.bh_datalen != (int)(sizeof(struct ip) + sizeof(struct udphdr) + datalen)) goto error;
    if (buf[BPF_WORDALIGN(sizeof(bh)) + sizeof(struct ip) + sizeof(struct udphdr)] != 'X') goto error;
    if (buf[size - 2] != 'Y') goto error;
    if (buf[size - 1] != 'Z') goto error;
    ...
    len = read(fd, buf, size);
    if (len <= 0) goto error;
    if (len >= (ssize_t)size * 3 / 4) goto error;
    if (test94_check(buf, len, 1, 1, NULL, NULL) != 2) goto error;
...
static void test94_comm(int fd, int fd2, int fd3, int filtered) {
    ...
    if (ioctl(fd, BIOCGSTATS, &bs) != 0) e(0);
    if (bs.bs_recv == 0 || bs.bs_capt == 0) e(0);

    if (ioctl(fd, BIOCFLUSH, 0) != 0) e(0);

    rw_ret = write(fd3, "B", 1);
    if (rw_ret != 1) e(0);
...
test94e(void)
{
    ...
    int i = 0, fd = -1, fd2 = -1, fd3 = -1, val = 1;
    ...
    if (test94_check(buf, len, 1, 1, caplen, datalen) != 5)
        e(0);
    ...
    if (test94_check(buf, len, 5, 1, caplen, datalen) != 9)
        e(0);
 ...
 */
static uint16_t test94_cksum(const uint8_t *buf, size_t len)
{
    uint32_t sum = 0;
    uint32_t word = 0;
    ...
    while (len > 1) {
        word = ((uint32_t)buf[0] << 8) | buf[1];
        sum += word;
        buf += 2;
        len -= 2;
    }
    if (len > 0) {
        word = ((uint32_t)buf[0] << 8);
        sum += word;
    }
...
static void test94f(void)
{
    ...
    void *tmp_buf = realloc(buf, UINT16_MAX + 1);
    if (tmp_buf == NULL) e(0);
    buf = tmp_buf;
    ...
    rlen = recv(fd3, buf, UINT16_MAX, 0);
    if (rlen != 12345) e(0);
    for (i = 0; i < 12345; i++) {
    ...
    memset(buf, 0, UINT16_MAX);
    rlen = recv(fd3, buf, UINT16_MAX, MSG_DONTWAIT);
    if (rlen != 12345) e(0);
    for (i = 0; i < 12345; i++) {
        if (buf[i] != 1 + (i % 251)) e(0);
    }
...
static void test94g(void)
{
    ...
    if (select(fd + 1, &rfds, &wfds, NULL, NULL) != 2 || !FD_ISSET(fd, &rfds) || !FD_ISSET(fd, &wfds)) {
        e(0);
        free(buf);
        close(fd);
        return;
...
static void test94h(void) {
    ...
    if (ioctl(cfd, BIOCGSTATS, &bs) != 0 || bs.bs_recv == 0 || bs.bs_drop == 0 || bs.bs_capt == 0) e(0);
    if (ioctl(cfd, BIOCGSTATS, &bs) != 0 || bs.bs_recv == 0 || bs.bs_drop == 0 || bs.bs_capt == 0) e(0);

    if (ioctl(cfd, FIONREAD, &val) != 0 || val == 0) e(0);
    ...
    bfl.bfl_list = list;
    if (ioctl(cfd, BIOCGDLTLIST, &bfl) != 0) e(0);
    if (bfl.bfl_len != 1 || bfl.bfl_list != list) e(0);
    if (list[0] != DLT_RAW || list[1] != 0) e(0);
    ...
    if (ioctl(ufd, BIOCGSEESENT, &uval) != 0 || uval != 1) e(0);
...
static void test94i(void)
{
    ...
    ssize_t len;
    size_t off;
    unsigned int uval, size = 0, dlt = 0;
    int fd = -1, fd2 = -1, fd3 = -1;
    int error = 0;
    ...
    len = read(fd, buf, size);
    if (len != (ssize_t)(BPF_WORDALIGN(sizeof(bh)) + BPF_WORDALIGN(sizeof(ip6) + sizeof(uh) + 1))) goto cleanup;

    memcpy(&bh, buf, sizeof(bh));

    if (bh.bh_tstamp.tv_sec == 0 && bh.bh_tstamp.tv_usec == 0) goto cleanup;
    if (bh.bh_caplen != sizeof(ip6) + sizeof(uh) + 1) goto cleanup;
    if (bh.bh_datalen != bh.bh_caplen) goto cleanup;
    if (bh.bh_hdrlen != BPF_WORDALIGN(sizeof(bh))) goto cleanup;

    if (buf[bh.bh_hdrlen + sizeof(ip6) + sizeof(uh)] != 'A') goto cleanup;

    off = test94_make_pkt(buf, 6, 1);
    memcpy(buf + off, "Hello!", 6);

    if (write(fd, buf, off + 6) != (ssize_t)(off + 6)) goto cleanup;
...
static void test94j(void)
{
    ...
    int fd = -1, fd2 = -1, fd3 = -1, val, mib[5], smib[3];
    int found;
...
static void test94l(void)
{
	...
	uint8_t buf[sizeof(struct ether_header) + (sizeof(struct ip6_hdr) > sizeof(struct ip) ? sizeof(struct ip6_hdr) : sizeof(struct ip)) + sizeof(struct udphdr) + 6];
	...
	for (ifp = ifa; ifp != NULL; ifp = ifp->ifa_next) {
		...
		if (ifdata != NULL &&
			...
			memcpy(ether.ether_dhost,
			       ((struct sockaddr_dl *)ifp->ifa_addr)->sdl_data + sdl.sdl_nlen,
			       sdl.sdl_alen);
			break;
		}
	}
